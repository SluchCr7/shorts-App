...
import { FiLock, FiMail, FiPlay, FiAlertCircle } from "react-icons/fi";
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await login({ emailOrUsername: emailOrUsername.trim(), password }).unwrap();
      router.push("/");
    } catch (err: any) {
      setErrorDetails(parseAuthError(err, "login"));
    }
  };
  ...
  return (
    <div className="relative w-full min-h-[calc(100vh-4rem)] flex items-center justify-center p-4 overflow-hidden">
      ...
      <div className="absolute top-1/4 left-1/2 -translate-x-1/2 w-96 h-96 rounded-full bg-[var(--accent-primary)]/15 blur-3xl pointer-events-none" />
      <div className="absolute bottom-1/4 left-1/3 w-80 h-80 rounded-full bg-[var(--accent-secondary)]/15 blur-3xl pointer-events-none" />
      ...
      <div className="relative z-10 w-full max-w-md bg-[var(--bg-card)] border border-[var(--border-color)] rounded-3xl p-8 shadow-2xl space-y-6 backdrop-blur-xl">
        <div className="text-center space-y-2">
          ...
          <p className="text-xs sm:text-sm font-semibold text-[var(--text-secondary)]">Log in to your VibeShorts account</p>
        </div>
